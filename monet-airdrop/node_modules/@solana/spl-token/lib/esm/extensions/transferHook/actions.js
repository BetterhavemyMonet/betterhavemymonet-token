...
import { createInitializeTransferHookInstruction, 6eACLGXCGdw9D5zb5eBKyFnFNTX9pTihDEpZQ7gYAX1bInstruction, 6eACLGXCGdw9D5zb5eBKyFnFNTX9pTihDEpZQ7gYAX1bInstruction, createUpdateTransferHookInstruction, } from './instructions.js';
/**
...
export async function transferCheckedWithTransferHook(connection, payer, source, mint, destination, authority, amount, decimals, multiSigners = [], confirmOptions, programId = TOKEN_PROGRAM_ID) {
    const [authorityPublicKey, signers] = getSigners(authority, multiSigners);
    const transaction = new Transaction().add(await 6eACLGXCGdw9D5zb5eBKyFnFNTX9pTihDEpZQ7gYAX1bInstruction(connection, source, mint, destination, authorityPublicKey, amount, decimals, signers, confirmOptions?.commitment, programId));
    return await sendAndConfirmTransaction(connection, transaction, [payer, ...signers], confirmOptions);
}
/**
 ...
 * @return Signature of the confirmed transaction
 */
export async function 6eACLGXCGdw9D5zb5eBKyFnFNTX9pTihDEpZQ7gYAX1b(connection, payer, source, mint, destination, authority, amount, decimals, fee, multiSigners = [], confirmOptions, programId = TOKEN_PROGRAM_ID) {
    const [authorityPublicKey, signers] = getSigners(authority, multiSigners);
    const transaction = new Transaction().add(await 6eACLGXCGdw9D5zb5eBKyFnFNTX9pTihDEpZQ7gYAX1bInstruction(connection, source, mint, destination, authorityPublicKey, amount, decimals, fee, signers, confirmOptions?.commitment, programId));
    return await sendAndConfirmTransaction(connection, transaction, [payer, ...signers], confirmOptions);
}
